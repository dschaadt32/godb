...
func (a *SumAggState[T]) Copy() AggState {
	return &SumAggState[T]{a.alias, a.expr, a.sum, a.null, a.getter}
}
...
func (a *AvgAggState[T]) Copy() AggState {
	return &AvgAggState[T]{a.alias, a.expr, a.sum, a.count, a.avg}
...
func (a *AvgAggState[T]) AddTuple(t *Tuple) {
	...
	switch any(a.sum).(type) {
	case string:
		fmt.Printf("a.sum should not be a string")
	default:
		val := intAggGetter(v)
		a.sum += int(val.(int64))
	}
